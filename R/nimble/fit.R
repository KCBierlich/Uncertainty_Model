fit = function(nim_pkg_file, niter, mcmc_sample_dir, nthin, useBarometer, 
               useLaser, length_prior = independent_lengths) {

  # load nimble package
  nim_pkg = readRDS(nim_pkg_file)
  
  # model specialization: altitude data
  nim_pkg$consts$useBarometer = useBarometer
  nim_pkg$consts$useLaser = useLaser
  
  # model specialization: length priors
  model.complete = length_prior(model = model, nim_pkg = nim_pkg)
  nim_pkg = model.complete$nim_pkg
  model = model.complete$model
  
  # initialize nimble model
  droneLengths = nimbleModel(code = model, name = id_chr(),
  ...
  if(!is.finite(droneLengths$calculate())) {
    ...
    message('Some nodes do not have finite initial likelihoods:')
    for(nom in names(ll)[which(!is.finite(ll))]) {
      print(nom)
      print(droneLengths[[nom]])
    }
  }
  ...
  # modify configuration according to length prior model
  if('configure_sampler' %in% names(length_prior)) {
    conf = length_prior$configure_sampler(conf)
  }
  ...
  saveRDS(list(samples = mcmc.out, config = nim_pkg), file = f)
