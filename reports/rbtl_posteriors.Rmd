...
```{r joint_posterior, fig.cap='Strong posterior correlation between regression parameters, as expected.  However, there is still strong posterior concentration in one region, which helps identify the population-level relationship.'}
...
slope_fig <- ggplot(data.frame(m), aes(x = beta.1., y = beta.2.)) + 
  ...
  theme_few() + 
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
ggarrange(est_fig, slope_fig, ncol = 1, labels = 'AUTO') 
ggsave(file.path("..", "figures", "AMW_RBTL.jpg"), width = 7, height = 7)
